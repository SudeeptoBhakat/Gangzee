<?php
 ...
 ?><div style="background-color: #050505;">
    <div class="container col-12 col-sm-10 col-lg-11 d-flex justify-content-between" style="background-color: #050505;">
        <div class="col-md-8 col-sm-12" style="max-width:93vw">       
            <?php
            showError('post_img');
            if(count($posts)<1){
                echo "<p style='width:100' class='p-2 bg-white border rounded text-center my-3 col-12'><img src='assets\images\1.png' alt=''><br>Follow Someone or Add a new post</p>";
            }
            foreach($posts as $post){
             ...
             <div class="card mt-4
             ">
                <div class="card-title d-flex justify-content-between align-items-center" style="background-color: hsl(0, 3%, 8%);">
                    ...
                    <div class="p-2">
                        <?php
                            if($post['uid']==$user['id']){
                              ...
                              <i class="bi bi-three-dots-vertical" style="color: white;" id="option<?=$post['id']?>" data-bs-toggle="dropdown" aria-expanded="false"></i>
                            
                              <ul class="dropdown-menu bg-dark" aria-labelledby="option<?=$post['id']?>">
                                <li><a class="dropdown-item text-light" href="assets/php/actions.php?deletepost=<?=$post['id']?>"><i class="bi bi-trash-fill" style="color: white;"></i> Delete Post</a></li>
                              </ul>
                            </div>
                                <?php
                            }
                ...
                <img src="assets/images/posts/<?=$post['post_img']?>" loading=lazy class="" style="background-color: hsl(0, 3%, 8%);" alt="...">
                <div style="background-color: hsl(0, 3%, 8%);">
                <h4 style="font-size: x-larger" class="p-2 d-flex">
               <span>
               <?php
                ...
                <i class="bi bi-heart like_btn text-light" style="display:<?=$like_btn_display?>" data-post-id='<?=$post['id']?>'></i>

                </span>
                &nbsp;&nbsp;<i class="bi bi-chat-left d-flex align-items-center text-light"><span class="p-1 mx-2 text-small" style="font-size:small" data-bs-toggle="modal" data-bs-target="#postview<?=$post['id']?>"><?=count($comments)?> Comments</span></i> 
                        
                </h4>
                </div>
                <div style="background-color: hsl(0, 3%, 8%);">
                <span class="p-1 mx-2 text-light" data-bs-toggle="modal" data-bs-target="#likes<?=$post['id']?>"><span id="likecount<?=$post['id']?>"><?=count($likes)?></span> likes</span>
                ...
                if($post['post_text']){
                    ?>
                 <div class="card-body text-light" style="background-color: hsl(0, 3%, 8%);">
                                <?=$post['post_text']?>
                                </div>
                    <?php
                }
                ?>
                <div class="input-group p-2 <?=$post['post_text']?'border-top':''?>" style="background-color: hsl(0, 3%, 8%);">
                 
                        <input type="text" class="form-control rounded border-0 comment-input" placeholder="Say Something...."
                                aria-label="Recipient's username" aria-describedby="button-addon2">
                            <button class="btn btn-outline-info rounded border-0 add-comment" data-page='wall' data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
  ...
  <div class="modal-content">
    ...
    ?>       
           ...
           <img src="assets\images\page-load-loader.gif" alt="loader" style="height: 100px;">
        </div>
        ...
        <div class="col-lg-4 col-sm-0 overflow-hidden mt-4 p-sm-0 p-md-3" >
            ...
            <div class="d-flex align-items-center p-2">
                ...
                <div>&nbsp;&nbsp;&nbsp;</div>
                <div class="d-flex flex-column justify-content-center">
                <a href='?u=<?=$user['username']?>' class="text-decoration-none text-light"><h6 style="margin: 0px;"><?=$user['first_name']?> <?=$user['last_name']?></h6></a>
                    <p style="margin:0px;" class="text-info">@<?=$user['username']?></p>
                </div>
            </div>
            ...
            <div>
                ...
                <?php
                    ...
                    <div class="d-flex align-items-center p-2">
                        ...
                        <div>&nbsp;&nbsp;</div>
                        <div class="d-flex flex-column justify-content-center">
                            <a href='?u=<?=$suser['username']?>' class="text-decoration-none text-light"><h6 style="margin: 0px;font-size: small;"><?=$suser['first_name']?> <?=$suser['last_name']?></h6></a>
                            <p style="margin:0px;font-size:small" class="text-info">@<?=$suser['username']?></p>
                        </div>
                    </div>
                    <div class="d-flex align-items-center">
                        <button class="btn btn-sm btn-info text-light followbtn" style="width: 140px;" data-user-id='<?=$suser['id']?>' >Follow</button>
